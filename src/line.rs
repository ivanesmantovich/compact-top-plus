...
pub fn tab_line(
    ...
    cols: usize,
    _toggle_tooltip_key: Option<String>,
    _tooltip_is_active: bool,
) -> Vec<LinePart> {
...
impl TabLinePrefixBuilder {
    ...
    fn build(&self, session_name: Option<&str>, mode: InputMode) -> Vec<LinePart> {
        ...
        if let Some(name) = session_name {
            ...
            let fallback_part = self.create_fallback_part();
            used_len += fallback_part.len;
    ...
    fn create_fallback_part(&self) -> LinePart {
        let prefix_text = "[] ";
    ...
    fn create_mode_part(&self, mode: InputMode, used_len: usize) -> Option<LinePart> {
        ...
        let mode_text = format!(" {} ", format!("{:?}", display_mode).to_uppercase());
...
impl RightSideElementsBuilder {
    ...
    fn build(&self, config: &TabLineConfig, available_space: usize) -> Vec<LinePart> {
        ...
        let used_space = calculate_total_length(&elements);
        let remaining_space = available_space.saturating_sub(used_space);

        if let Some(swap_status) = self.create_swap_layout_status(config, remaining_space) {
            elements.push(swap_status);
        }

        let used_space = calculate_total_length(&elements);
        let time = self.create_time_indicator();
        if used_space + time.len <= available_space {
            elements.push(time);
        }
